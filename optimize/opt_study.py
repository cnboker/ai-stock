...
def run_optuna_study(
    ...
    if slope_stats:
        # 记录 95 分位最大值和平均值
        study.set_user_attr(
            "dynamic_max_slope", float(slope_stats.get("max_ref", 0.02))
        )
        study.set_user_attr(
            "dynamic_avg_slope", float(slope_stats.get("avg_ref", 0.001))
        )
    ...
    best_report = DiagnosticScanner.run_body_check(
        ticker, ticker_period, study.best_params
    )
    ...
    PersistManager.save_best_config(
        study, ticker
    )  # 可选：将整个 study 存档到文件系统或数据库
...
def objective(trial, ticker: str, ticker_period="30"):
    ...
    with use_config(optuna_config):
        try:
            ...
            print(
                f"Trial {trial.number} - {ticker} | Train Stats: {train_stats} | Test Stats: {test_stats}"
            )
            if (
                train_stats.get("is_valid") == 0
                or test_stats.get("is_valid") == 0
            ):
                return -200 #无效记录
            ...
            print(
                f"🚀 Trial {trial.number}: {ticker} 得分 = {ticker_score:.2f} (Train: {train_val:.2f}, Test: {test_val:.2f}, Alpha: {test_alpha:.2f}%)"
            )
