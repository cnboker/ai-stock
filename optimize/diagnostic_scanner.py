...
class DiagnosticScanner:
    @staticmethod
    def run_body_check(ticker, test_config):
        """
        ...
        final_inject_config = {
            k.replace(f"{category}_", "").upper(): v 
        ...
        with use_config(final_inject_config):
            ...
            for day in trade_dates:
                try:
                    ...
                    pre_result = run_prediction(df=ticker_df, hs300_df=runner.get_history(day, runner.hs300_df),
                                               ticker=ticker, period="60", eq_feat=None)
                    ...
                    slope = (median_prices[-1] - median_prices[0]) / len(median_prices) / current_price
                    ...
                    if conf < settings.MODEL_TH:
                        ...
                        if strength < 0.01: # 认定为“强度幽灵”，即信号存在但无法开仓
                            ...
                            match_count += 1
                            sample_details.append({"day": day, "slope": slope, "strength": strength})

                except Exception as e:
                    reject_reasons["other_error"] += 1

        # 4. 构建标准化报告
        report = {"success_count": match_count, "intercept_report": reject_reasons, "scan_days": len(trade_dates)}
        
        # 5. 🔥 深度调参指南打印
        DiagnosticScanner._print_doctor_advice(ticker, match_count, reject_reasons, final_inject_config)
    ...
    def _print_doctor_advice(ticker, match_count, reasons, config):
        ...
        if reasons["zero_strength_ghost"] > 0:
            ...
            print(f"   原因：当前 STRENGTH_ALPHA ({config.get('STRENGTH_ALPHA')}) 过大，过度抑制了小斜率信号。")
            print(f"   👉 建议：将 initial_trial 中的 'strength_alpha' 调低至 0.2 ~ 0.5 之间。")
        ...
        if match_count > 0:
            ...
            if current_atr < 4.5:
                print(f"   警惕：当前 ATR_STOP ({current_atr}) 对 {ticker} 来说可能过紧。")
                print(f"   👉 建议：若回测显示“秒止损”，请将 'atr_stop' 提升至 4.5 - 5.5。")

        # C. 诊断 模型门槛
        if reasons["model_confidence"] > 10:
            current_th = config.get('MODEL_TH', 0.5)
            print(f"🚩 [信号稀缺] 模型置信度拦截了大部分行情。")
            print(f"   👉 建议：尝试将 'model_th' 调低 0.05（当前: {current_th} -> 目标: {current_th - 0.05:.2f}）。")

        # D. 诊断 确认天数
        current_n = config.get('CONFIRM_N', 3)
        if current_n > 1:
            print(f"🚩 [时效提示] 当前 CONFIRM_N={current_n}。")
            print(f"   👉 建议：对于爆发性标的，务必确认 'confirm_n' 已设为 1，否则信号会被延时磨灭。")
