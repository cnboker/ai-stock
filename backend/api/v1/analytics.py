...
async def get_daily_review(
    ...
    try:
        ...
        return generate_review_data(predictions,orders)
...
def generate_review_data(predictions, orders):
    ...
    def to_dt(ts):
        if isinstance(ts, str):
            # 兼容带毫秒和不带毫秒的格式
            try: return datetime.strptime(ts, "%Y-%m-%d %H:%M:%S.%f")
            except: return datetime.strptime(ts, "%Y-%m-%d %H:%M:%S")
        return ts
    for i, pred in enumerate(sorted_preds):
        ...
        next_same_symbol_pred = next(
            (p for p in sorted_preds[i+1:] if p.symbol == pred.symbol), 
            None
        )
        end_time = to_dt(next_same_symbol_pred.timestamp) if next_same_symbol_pred else datetime.max

        # 优化点 2：容错性匹配
        # 交易执行往往比信号慢几毫秒，所以用 >= 是对的
        matches = [
            o for o in realized_orders 
